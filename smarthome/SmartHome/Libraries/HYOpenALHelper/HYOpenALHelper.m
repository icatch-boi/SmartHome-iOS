...
//                                                        userInfo:nil repeats:YES];
    ...
    self.freq = freq;
    SHLogInfo(SHLogTagAPP, @"Audio Frequency: %d", freq);
    if (channel == 1 && bit == 8) {
        SHLogInfo(SHLogTagAPP, @"mono08");
        self.format = AL_FORMAT_MONO8;
    } else if (channel == 1 && bit == 16) {
        SHLogInfo(SHLogTagAPP, @"mono16");
        self.format = AL_FORMAT_MONO16;
    } else if (channel == 2 && bit == 8) {
        SHLogInfo(SHLogTagAPP, @"stereo08");
        self.format = AL_FORMAT_STEREO8;
    } else if (channel == 2 && bit == 16) {
        SHLogInfo(SHLogTagAPP, @"stereo16");
        self.format = AL_FORMAT_STEREO16;
    } else {
        SHLogInfo(SHLogTagAPP, @"Unknow channel: %d", channel);
    }
    return YES;
}
...
- (BOOL)insertPCMDataToQueue:(const ALvoid *)data size:(NSUInteger)size
{
    @synchronized(self) {
        ...
        if (error != AL_NO_ERROR) {
            SHLogError(SHLogTagAPP, @"** Insert PCM data failed with error code %d.", error);
            return NO;
        }

        if (data == NULL) {
            SHLogError(SHLogTagAPP, @"** PCM data is null.");
            return NO;
        }
        ...
        error = alGetError();
        if (error != AL_NO_ERROR) {
            SHLogError(SHLogTagAPP, @"** Failed to create buffer with error code %d.", error);
            return NO;
        }
        ...
        error = alGetError();
        if (error != AL_NO_ERROR) {
            SHLogError(SHLogTagAPP, @"** Copy failed with error code %d.", error);
            return NO;
        }
        ...
        if (state == AL_STOPPED) {
            SHLogWarn(SHLogTagAPP, @"openAL 播放中断......");
            [self getInfo2];
            [self cleanBuffers];
        ...
        error = alGetError();
        if (error != AL_NO_ERROR) {
            SHLogError(SHLogTagAPP, @"** Insert failed with error code %d.", error);
            return NO;
        }
...
- (void)play
{
    ALint state;
    alGetSourcei(outSourceID, AL_SOURCE_STATE, &state);
    SHLogDebug(SHLogTagAPP, @"state: %x", state);
    if (state != AL_PLAYING) {
...
-(double)timestamp
{
    ...
    if((error = alGetError()) != AL_NO_ERROR) {
        if (error == AL_INVALID_VALUE) {
            SHLogError(SHLogTagAPP, @"error OFFSET source: AL_INVALID_VALUE:%f",sec);
        }else if (error == AL_INVALID_ENUM) {
            SHLogError(SHLogTagAPP, @"error OFFSET source: AL_INVALID_ENUM");
        }else if (error == AL_INVALID_NAME) {
            SHLogError(SHLogTagAPP, @"error OFFSET source: AL_INVALID_NAME");
        }else if (error == AL_INVALID_OPERATION) {
            SHLogError(SHLogTagAPP, @"error OFFSET source: AL_INVALID_OPERATION");
        }else {
            SHLogError(SHLogTagAPP, @"error OFFSET source: %x", error);
        }
        
    } else {
        SHLogInfo(SHLogTagAPP, @"OFFSET source: %f, removed: %f", sec, _removedBufferLength);
