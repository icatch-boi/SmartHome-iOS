...
- (instancetype)initWithIsVideo:(BOOL)isVideo isCallee:(BOOL)isCallee inView:(UIView *)view
{
...
- (void)initUIForAudioCallee
{
    ...
    CGFloat btnW = kBtnW;
    CGFloat btnH = kBtnW + 30;
    CGFloat paddingX = (kRTCWidth - btnW * 2) / 4;
    self.hangupBtn.frame = CGRectMake(paddingX, kContainerH - btnH - 5 + 45, btnW, btnH);
    [self.btnContainerView addSubview:_hangupBtn];
    
    self.answerBtn.frame = CGRectMake(paddingX * 3 + btnW, kContainerH - btnH - 5 + 45, btnW, btnH);
    [self.btnContainerView addSubview:_answerBtn];
...
- (void)initUIForTopCommonViews
{
    ...
    self.btnContainerView.frame = CGRectMake(0, kRTCHeight - kContainerH - 100, kRTCWidth, kContainerH);
    [self addSubview:_btnContainerView];
}
...
- (void)initLoadingView {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (i == 0) {
            images = @[loadingImg_Highlighted, loadingImg_Default, loadingImg_Default];
        } else if (i == 1) {
            images = @[loadingImg_Default, loadingImg_Highlighted, loadingImg_Default];
        } else if (i == 2) {
            images = @[loadingImg_Default, loadingImg_Default, loadingImg_Highlighted];
        }
...
- (RTCButton *)hangupBtn
{
    if (!_hangupBtn) {
        if (_callee && !_answered) {
            _hangupBtn = [[RTCButton alloc] initWithTitle:/*@"Refuse"*/NSLocalizedString(@"kRefuseRing", nil) /*@"拒绝"*/  noHandleImageName:@"icon_call_reject_normal"];
        } else {
...
{
    if (!_answerBtn) {
        _answerBtn = [[RTCButton alloc] initWithTitle:/*@"Accept"*/NSLocalizedString(@"kAcceptRing", nil) /*@"接听"*/ noHandleImageName:@"icon_audio_receive_normal"];
