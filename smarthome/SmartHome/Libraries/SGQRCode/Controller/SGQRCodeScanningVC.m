...
- (void)viewWillAppear:(BOOL)animated{
	[self setupSGQRCodeScanning];//to sovle bug that the background is back when reback from scan result
}
...
- (void)setupNavigationBar {
    self.navigationItem.title = NSLocalizedString(@"kScan", nil);
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"CameraAlbum", nil) style:(UIBarButtonItemStyleDone) target:self action:@selector(rightBarButtonItenAction)];
}
