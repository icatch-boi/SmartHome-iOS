...
/*
 ...
 dialogView.center = CGPointMake(CGRectGetMidX(bounds), CGRectGetMidY(bounds));
...
- (id)init
{
    ...
    if (self) {
        ...
        buttonTitles = @[NSLocalizedString(@"ALERT_CLOSE", nil)];
...
- (UIView *)createContainerView
{
    if (containerView == NULL) {
        containerView = [[UIView alloc] initWithFrame:CGRectMake(0, 50, 300, 150)];
    } else {
        containerView.frame = CGRectMake(0, 50, 290, 150);
    }
...
*/
    ...
    // There is a line below the alertview's title
    lineView = [[UIView alloc] initWithFrame:CGRectMake(0, 50, dialogContainer.bounds.size.width, buttonSpacerHeight)];
    lineView.backgroundColor = [UIColor colorWithRed:198.0/255.0 green:198.0/255.0 blue:198.0/255.0 alpha:1.0f];
    [dialogContainer addSubview:lineView];
    // ^^^
    
    UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 6, dialogContainer.bounds.size.width, 36)];
    titleLabel.text = _title;
    titleLabel.textColor = [UIColor colorWithRed:0.0f green:0.5f blue:1.0f alpha:1.0f];
...
- (CGSize)countDialogSize
{
    ...
    CGFloat dialogHeight = containerView.frame.size.height + buttonHeight + buttonSpacerHeight + 50;
...
- (CGSize)countScreenSize
{
    ...
    UIInterfaceOrientation interfaceOrientation = [[UIApplication sharedApplication] statusBarOrientation];
    if (SYSTEM_VERSION_LESS_THAN(@"8.0") && UIInterfaceOrientationIsLandscape(interfaceOrientation)) {
