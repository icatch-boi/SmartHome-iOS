...
//	........................
	...
	if ([self numberOfPhotos] > 0 && self.isImage == YES){
...
- (void)cameraDisconnectHandle:(NSNotification *)nc {
    if ([self.delegate respondsToSelector:@selector(photoBrowser:cameraDisconnectHandle:)]) {
        [self.delegate photoBrowser:self cameraDisconnectHandle:nc];
    }
}

- (void)cameraPowerOffHandle:(NSNotification *)nc {
    if ([self.delegate respondsToSelector:@selector(photoBrowser:cameraPowerOffHandle:)]) {
        [self.delegate photoBrowser:self cameraPowerOffHandle:nc];
    }
}
...
- (void)actionButtonPressed:(id)sender {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             ...
                                             name    :@"kCameraDownloadCompleteNotification"
                                             object  :nil];
...
//                [self showDownloadConfirm];
                dispatch_queue_t shareQueue = dispatch_queue_create("Smarthome.GCD.Queue.Playback.Share", 0);
...
{
    BOOL ret = [notification.object intValue];
    dispatch_async(dispatch_get_main_queue(), ^{
